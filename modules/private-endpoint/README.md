...
Bicep Module for provisioning a Private Endpoint with optional DNS Zone Group integration following a configurable naming convention (`{workloadName}-pep-{environment}`). The `name` parameter allows you to completely override the automatic name. Allows connecting Azure resources via private link, with automatic DNS registration when a private DNS zone is provided.

## Usage

```bicep
// Generates: myapp-pep-dev
